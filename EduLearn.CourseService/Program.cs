...
// ── Redis Cache (PDF Non-Functional: 5-min TTL for popular courses) ───────────
// Falls back gracefully if Redis not running — app still works without it
builder.Services.AddStackExchangeRedisCache(options =>
{
    options.Configuration         = builder.Configuration["Redis:ConnectionString"]
                                    ?? "localhost:6379";
    options.InstanceName          = "EduLearn_Course_";
});
...
builder.Services.AddSwaggerGen(c =>
{
    c.SwaggerDoc("v1", new OpenApiInfo
    {
        ...
        Version     = "v1",
        Description = "Course catalogue with Redis caching (5-min TTL)"
    });
